...
@section('title')
{{--@section('title',$article->id?'文章修改':'文章添加')--}}
@endsection
@section('css')
                ...
                @else
                    <form method="post" action="{{route('admin.articles.store')}}"
                        ...
                        <div class="row cl">
                            ...
                            <div class="formControls col-xs-8 col-sm-9">
                                <!-- onKeyUp="$.Huitextarealength(this,200)" datatype="*10-100" dragonfly="true" nullmsg="备注不能为空！" -->
                                <textarea name="digest" cols="" rows="" class="textarea"
                                          placeholder="说点什么...最少输入10个字符">{{$article->digest??''}}</textarea>
                        ...
                        <div class="row cl">
                            ...
                            <div class="formControls col-xs-8 col-sm-9">
                                <input type="text" class="input-text" value="{{$article->source??''}}" placeholder=""
                            ...
                            <div class="formControls col-xs-8 col-sm-9">
                                <div class="uploader-thum-container">
                                    <div id="fileList" class="uploader-list">
                                        @if($article->url)
                                            <div id="" class="item upload-state-success">
                                                <div class="pic-box"><img width="100px" height="100px"
                                                                          src="{{$article->url}}"></div>
                                                <div class="info">原图</div>
                                                {{--<p class="state">原始图片</p>--}}
                                            </div>
                                        @endif
                                    </div>
    ...
    <script type="text/javascript">
        $(function () {
            ...
            var uploader = WebUploader.create({
                ...
                // 文件接收服务端。
                server: '{{route('admin.articles.uploads')}}',
            ...
            uploader.on('uploadSuccess', function (file, filepath) {
                // console.log(filepath);
                $('#' + file.id).addClass('upload-state-success').find(".state").text("已上传");
                // $('#' + file.id).addClass('upload-state-done');
                $('input[name=url]').val(filepath.url);
